...
export function StoreSelector() {
    ...
    if (!isAdmin) {
        return (
            <div className="w-full p-3 bg-brand-50/50 rounded-xl border border-brand-100 flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center text-brand-600 shrink-0">
                    <MapPin size={16} />
                </div>
                <div className="overflow-hidden">
                    <p className="text-[10px] text-brand-400 font-semibold uppercase tracking-wider mb-0.5">Current Location</p>
            ...
            >
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="flex flex-col items-start overflow-hidden">
                        ...
                        <span className="text-sm font-semibold text-gray-700 truncate w-full text-left">
                            {selectedStore ? selectedStore.name : 'Headquarters'}
                        </span>
                    </div>
                </div>
                <ChevronDown size={16} className={`text-gray-400 ml-2 transition-transform duration-200 ${isOpen ? 'rotate-180 text-brand-500' : ''}`} />
            </button>
                    ...
                    <div className="max-h-64 overflow-y-auto p-1.5 space-y-0.5">
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${!selectedStoreId ? 'bg-brand-50' : 'hover:bg-gray-50'}
                            `}
                        >
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${!selectedStoreId ? 'bg-white text-brand-600 shadow-sm' : 'bg-gray-100 text-gray-400 group-hover:bg-white group-hover:shadow-sm'}`}>
                                <Building2 size={16} />
                            </div>
                            <div className="flex-1 text-left">
                                <p className={`text-sm font-semibold ${!selectedStoreId ? 'text-brand-900' : 'text-gray-700'}`}>Headquarters</p>
                                <p className="text-[10px] text-gray-400">View All Data</p>
                            </div>
                            {!selectedStoreId && <Check size={16} className="text-brand-600" />}
                        </button>
                        ...
                        {stores.map(store => {
                            ...
                            return (
                                <button
                                    ...
                                    `}
                                >
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-white text-brand-600 shadow-sm' : 'bg-gray-100 text-gray-400 group-hover:bg-white group-hover:shadow-sm'}`}>
                                        <MapPin size={16} />
                                    </div>
                                    <div className="flex-1 text-left overflow-hidden">
                                        <p className={`text-sm font-semibold truncate ${isSelected ? 'text-brand-900' : 'text-gray-700'}`}>{store.name}</p>
                                    </div>
